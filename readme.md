...
- 実行させるOSによってそれに合った設定に変更しなければなりません。例えば、通常のWindows環境と、WSL2(Linux)上における設定は違うことがあります。

- Unix系OSを用いてCPU時間(mainTimeMs)を測定するとき、gcc環境があれば1ms単位で測定が可能です。なければ10ms単位になります。


- ~~**重要** : PowershellのCPU時間が正しく計測できないことから、`NPC : Run Judge`はWindows上で動作させるには不十分です。特に、なぜかv0.2.8でばぐりました~~
この問題はv.0.2.11で解決されました。ネイティブWindowsにおいては、分解能が1/64秒であることから、実行時間はおおよそ16ms毎の値になります。
## 設定から変更可能なもの
...
- `Timeout Ms Gen`
    ...
    生成コードの上限実行時間を設定する。この時間を超えると強制的に操作が打ち切られる。デフォルトは`30000`
...
- `Timeout Ms Scorer`
    ...
    評価コードの上限実行時間を設定する。この時間を超えると強制的に操作が打ち切られる。デフォルトは`30000`
...
|2026/01/19|無限ループするようなコードに対して、実行を確実に切れるように修正|0.2.8|
|2026/01/24|ヒューリスティックにおいて、無限ループするような実行を強制的に切った後、ジャッジの再試行を指定された回数だけ行うように修正|0.2.9|
|2026/01/25|WSLなどのUnixOSにおける実行で、CPU時間測定を1ms単位で行えるように変更|0.2.10|
|2026/01/25|windowsでもCPU時間(mainTimeMs)が16ms単位で取得できるように改善|0.2.11|